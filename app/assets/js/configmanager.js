...
const dataPath = path.join(sysRoot, BrandConfig.gameDataDirectoryName)
...
exports.load = function(){
    ...
    if(doLoad){
        ...
        if(doValidate){
            ...
            if(!config || typeof config !== 'object') { config = Object.assign({}, DEFAULT_CONFIG) }
            exports.save()
        }
    }
    // Final safety net
    if(!config) config = Object.assign({}, DEFAULT_CONFIG)
    if(!config.authenticationDatabase) config.authenticationDatabase = {}
    if(!config.settings) config.settings = DEFAULT_CONFIG.settings
